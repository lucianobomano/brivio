...
export const mixWithWhite = (hex: string, percentage: number) => {
    const rgb = hexToRgb(hex);
    if (!rgb) return hex;

    const r = Math.round(rgb.r + (255 - rgb.r) * (1 - percentage / 100));
    const g = Math.round(rgb.g + (255 - rgb.g) * (1 - percentage / 100));
    const b = Math.round(rgb.b + (255 - rgb.b) * (1 - percentage / 100));
